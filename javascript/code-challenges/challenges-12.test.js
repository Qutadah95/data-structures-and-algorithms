...
const validateEmail = (email) => {
  // Solution code here...
  let email0 = /^\w+(\.\w+)?@\w+\.(net|org|com)$/;

  if (email0.test(email)){
    return true ;
  } else{
    return false ;
  }
...
const validatePhoneNumber = (phoneNumber) => {
  // Solution code here...
  let x = /^((\(\d{3}\))|(\d{3}))[ -]?\d{3}[ -]?\d{4}$/;

  if (x.test(phoneNumber)) {
    return true ;
  } else{
    return false ;
  }
